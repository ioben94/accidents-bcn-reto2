...
El principal objetivo es analizar la evolución y la distribución de la siniestralidad vial gestionada por la Guàrdia Urbana entre el año 2019 y el 2025 para determinar si la reducción del número de accidentes se ha acompañado por una reducción de su gravedad y qué zonas y momentos del día concentran el riesgo más alto. 
...
tibble(
  ...
  filter(!Variable %in% c("fichero_origen", "nom_mes",
                          "descripcio_dia_setmana", "numero_expedient",
                          "codi_districte", "total_lesionados",
                          "accidente_con_victimas", "es_causa_vianant")) |>
...
(2) Variables territoriales. El distrito y el barrio son categóricas nominales, de diez y 73 niveles respectivamente; el nombre de la calle alcanza 6.031 valores distintos. Las coordenadas longitud y latitud son numéricas continuas expresadas en grados decimales sobre el sistema WGS84, con rango limitado al término municipal. 
...
Los dos gráficos siguientes deben leerse juntos, porque cuentan una historia que ninguno de ellos revela por separado.
...
El Eixample concentra 15.080 accidentes, más de una cuarta parte de los siniestros de toda la ciudad muy por delante de Sant Martí (6.872) y Sants-Montjuïc (6.168). Es un resultado esperado ya que es el distrito más poblado, más denso y atravesado por los principales ejes viarios.

En el primer gráfico podemos observar que cuando se atiende a la proporción de accidentes graves o mortales, el Eixample cae a la zona media con un 2,59 %, prácticamente en la media municipal del 2,62 %. Quien encabeza el ranking es Horta-Guinardó, con un 3,41 %, siendo apenas el quinto distrito en volumen. Le sigue Les Corts con un 3,06 %. En el extremo opuesto, Sant Andreu presenta la tasa más baja de la ciudad (1,96 %), seguido de Gràcia (2,01 %).

El segundo gráfico confirma que no se trata de una casualidad: la correlación de Pearson entre el número de accidentes de un distrito y su gravedad relativa es de 0,097, prácticamente nula. Esto nos dice que volumen y riesgo son dimensiones independientes.
...
Las dos primeras abordan la dimensión temporal y están deliberadamente separadas: parecen la misma cuestión, pero el análisis exploratorio ha demostrado que sus respuestas divergen y formularlas por separado obliga al panel a mostrar ambas magnitudes. La tercera y la cuarta se centran en dónde y cuándo intervenir, que es lo que un técnico necesita para justificar una actuación, y la quinta desciende al nivel de detalle máximo que permiten los datos. La sexta merece un comentario aparte porque es la que justifica la interacción. Si todas fueran de alcance municipal, un panel estático bastaría. Al preguntar si los patrones de riesgo son iguales en todos los distritos, se hace necesario que el usuario pueda filtrar y que ese filtro se propague a todos los gráficos.
...
Se ha optado por un gráfico de líneas: el eje horizontal representa el tiempo, variable continua ordenada y el vertical el número de accidentes, numérica discreta en escala de razón. La magnitud se codifica mediante la posición vertical, la codificación perceptivamente más precisa según la jerarquía de Cleveland y McGill (1984) y la línea aprovecha la ley Gestalt de continuidad.

El eje vertical empieza en cero. En un gráfico de líneas la decisión es discutible, pero se ha mantenido porque el mensaje es precisamente la magnitud de la caída de 2020 y truncar el eje la exageraría.
...
Hora del día y día de la semana se sitúan en los ejes y la frecuencia se codifica mediante la saturación con una escala secuencial monocroma: al tratarse de un valor numérico ordenado, el color debe variar en intensidad y no en tono, ya que un cambio de tono sugeriría categorías distintas. El fundamento es que la intensidad es una propiedad preatentiva, de modo que las zonas de mayor concentración se detectan sin recorrer la cuadrícula celda por celda.
...
Durante la construcción del prototipo detecté un problema que señala que las vistas vinculadas no deben aplicarse de forma automática a todos los elementos.
...
La interacción no es gratuita, cada control exige del usuario una decisión y una carga de memoria. Se han adoptado cinco medidas para mitigarla:
(1) Todos los filtros concentrados en un único lugar.
(2) Estado inicial que responde ya a la pregunta principal sin interactuar.
(3) Controles con convenciones estándar, sin mecanismos que exijan aprendizaje.
(4) Título de cada gráfico reflejando el filtro activo, como retroalimentación.
(5) Botón de reinicio para volver al estado inicial.

Se ha descartado la navegación jerárquica hasta el nivel de barrio: añadiría un nivel que la audiencia no necesita.
